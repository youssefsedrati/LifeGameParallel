...
echo '# nb_processes 4096_local 8192_local 16384_local 32768_local 4096 8192 16384 32768' > data/benchMPI_OMP_SYNC.dat
...
do
    t=`NUM_THREADS=16 mpiexec -np 1 --bind-to none ./main MPI_OMP_SYNC 100 ${s}`
    localLine="${localLine} ${t}"
done
echo "1 ${localLine} ${localLine}" >> data/benchMPI_OMP_SYNC.dat


for nbProcesses in 2 4 8
do
    line="${nbProcesses} ${localLine}"
    for s in 4096 8192 16384 32768
    do
        time=`NUM_THREADS=16 mpiexec -np ${nbProcesses} --bind-to none ./main MPI_OMP_SYNC 100 ${s}`
        line="${line} ${time}"
    done
